...
def extract_embeddings(model):
    ...
    if hasattr(model, "roberta"):
        return model.roberta.embeddings.word_embeddings.weight.detach().cpu().numpy()

    if hasattr(model, "distilbert"):
        return model.distilbert.embeddings.word_embeddings.weight.detach().cpu().numpy()
...
# ================= MASKED BEHAVIOR =================
def compute_masked_behavior(model_base, model_ft, tokenizer, probe_prompts):
    ...
    for prompt in probe_prompts:
        ...
        logits_base = out_base.logits[0, mask_index, :].squeeze(0)
        logits_ft = out_ft.logits[0, mask_index, :].squeeze(0)
...
# ================= MAIN ENGINE =================
def run_encoder_analysis(baseline_path, finetuned_path, probe_prompts):

    print("Loading encoder models...")
    tokenizer = AutoTokenizer.from_pretrained(baseline_path)
    model_base = AutoModelForMaskedLM.from_pretrained(baseline_path)
    model_ft = AutoModelForMaskedLM.from_pretrained(finetuned_path)
    ...
    print("Computing masked behavioral drift...")
    ...
    (
        ...
        ft_entropies,
        entropy_delta
    ) = compute_masked_behavior(model_base, model_ft, tokenizer, probe_prompts)
